...
from scrat.conf.config import myapp 
...
class powDecNew():
    ...
    def has_many(self, child_as_str, backref=False):
        ...
        def decorator(parent_class):
            ...
            import sys
            if "scrat.models.sql." + child_module_name in sys.modules.keys():
                #print(dir(sys.modules["scrat.models.sql." + child_module_name]))
                child_klass = getattr(sys.modules["scrat.models.sql." + child_module_name], child_class_name)
            else:
                import importlib
                mod = importlib.import_module('scrat.models.sql.' + child_module_name)
                #mod = __import__('scrat.models.sql.'+rel_module_name, fromlist=[rel_class_name])
    ...
    def belongs_to(self, parent_as_str):
        ...
        def decorator(child_class):
            ...
            parent_class = None
            import sys
            if ("scrat.models.sql."+parent_module_name) in sys.modules.keys():
                parent_class = getattr(sys.modules["scrat.models.sql."+parent_module_name], parent_class_name)
            else:
                import importlib
                mod = importlib.import_module('scrat.models.sql.' + parent_module_name)
                #mod = __import__('scrat.models.sql.'+rel_module_name, fromlist=[rel_class_name])
    ...
    def one_to_one(self, child_as_str):
        ...
        def decorator(parent):
            ...
            mod = __import__('scrat.models.sql.'+child_module_name, fromlist=[child_class_name])
