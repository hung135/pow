...
class TestClass:
    ...
    def test_server(self):
        ...
        import scrat.server
        import requests
        import scrat.conf.config as cfg
        import time
        p = Process(target=scrat.server.main)
        p.start()
    ...
    def test_sql_generate_model(self):
        ...
        import scrat.generate_model as gm
        import uuid
        import os.path
        ret = gm.generate_model(MODELNAME, "sql", appname="scrat")
    ...
    def test_sql_model_type(self):
        ...
        from scrat.models.sql.pow_test_model import PowTestModel
        m = PowTestModel()
    ...
    def test_sql_dbsetup(self):
        ...
        import scrat.init_sqldb_environment
        import os
        os.chdir("..")
        r = scrat.init_sqldb_environment.init_migrations()
        assert r == True
    ...
    def test_sql_migration(self):
        ...
        import scrat.generate_migration
        import os
        os.chdir("..")
        script = scrat.generate_migration.generate_migration(message="pow_test")
    ...
    def test_sql_dbupdate(self):
        ...
        import scrat.update_db
        import os, time
        ret = None
        os.chdir("..")
        time.sleep(1)
        try:
            ret = scrat.update_db.migrate("up")
    ...
    def test_tinydb_generate_model(self):
        ...
        import uuid
        import os.path
        ret = gm.generate_model(MODELNAME, "tinydb", appname="scrat")
        # generate model returns true in case of success
        assert ret is True
    ...
    def test_if_tinydb_model_validation_works(self):
        ...
        """ 
        print(" .. Test SQL: model.upsert() and model.find()")
        from scrat.models.tinydb.pow_test_model import PowTestModel
        m = PowTestModel()
        assert m.validate() == True
